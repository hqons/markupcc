    ...
    if (node.name == "div")
    {
        ...
        for (auto &child : node.children)
        {
            ...
            recursion_body_code(child, body_codes, div_vars, div_info_map); // 传递 div_info_map
             ...
             node.name == "h4" || node.name == "h5" || node.name == "h6" || node.name == "button"|| node.name == "grid")
    {
...
inline void emit_ui_element_code(
    std::ostringstream &out,
    const std::string &parent_var,
    const body_code &code,
    const std::string &id,
    const std::string &cssclass,
    const std::string &font_var = "font")
{
    switch (code.body_type)
    {
    case body_type::Paragraph:
        out << parent_var << ".addParagraph(\"" << escape_text(code.text)
            << "\", " << font_var << ", " << code.font_size
            << ", \"" << id << "\", \"" << cssclass << "\");\n";
        break;

    case body_type::Button:
        out << parent_var << ".addButton(\"" << escape_text(code.text)
            << "\", " << font_var
            << ", \"" << id << "\", \"" << cssclass << "\");\n";
        break;

    case body_type::Grid:
        std::string var_name=generate_uuid_var("grid");
        out <<"auto "<<var_name<<"="<< parent_var << ".addGrid(600, 200, \"" << id << "\", \"" << cssclass << "\");\n";

        // 插入 grid 子元素
        for (const auto &child : code.self_node->children)
        {
            if (child.name == "p" || child.name == "button" ||
                child.name == "h1" || child.name == "h2" || child.name == "h3" ||
                child.name == "h4" || child.name == "h5" || child.name == "h6")
            {

                std::string child_text = escape_text(child.content);
                std::string child_id = child.attrs.count("id") ? child.attrs.at("id") : "";
                std::string child_class = child.attrs.count("class") ? child.attrs.at("class") : "";

                if (child.name == "button")
                {
                    out << var_name << "->addChild(Element(new Button(\"" << child_text << "\", " << font_var << ", 0, 0, \"" << child_id << "\", \"" << child_class << "\")));\n";
                }
                else
                {
                    int font_size = 16;
                    if (child.name == "h1")
                        ...
                        font_size = 13;
                    else if (child.name == "h6")
                        font_size = 11;

                    out << id << ".addChild(Element(new Paragraph(\"" << child_text
                        << "\", " << font_var << ", " << font_size
...
void compile(mkml_node &mkml, const std::string &maincpp_path)
{
    ...
    {
        std::string id = code.attrs.count("id") ? code.attrs.at("id") : "";
        std::string cssclass = code.attrs.count("class") ? code.attrs.at("class") : "";

        emit_ui_element_code(body_code_out, "rootdiv", code, id, cssclass);
    }
    ...
    for (const auto &pair : div_map)
    {
        ...
        for (const auto &code : pair.second)
        {
            ...
            emit_ui_element_code(body_code_out, var, code, id, cssclass);
        }
